...
const List = ({results, fromMyPokemon = false}) => {
  ...
  const pokemonName = (pokemon) => {
    ...
    name += pokemon.fibonacci !== undefined ? `-${pokemon.fibonacci}` : '';
      ...
      {results?.map((pokemon, key) => {
        ...
        return (
          <div className='text-center' key={key}>
            <Link to={`/pokemon/${fromMyPokemon ? pokemon.id : pokemonArray[6]}/detail`} className="block p-6 max-w-sm bg-white rounded-lg border border-gray-200 shadow-md hover:bg-gray-100">
